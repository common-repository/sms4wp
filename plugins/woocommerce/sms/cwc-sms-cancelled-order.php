...
if ( ! class_exists( 'WC_SMS_Cancelled_Order' ) ) :

/**
 * 새로운 주문에 SMS 보내기
 */
class WC_SMS_Cancelled_Order extends sms4wpAddOn {
	private $section;
	/**
	 * Constructor
	 */
	function __construct() {
		$this->section = 'wc_cancelled_order';

		// Triggers for this sms
		// add_action( 'woocommerce_order_status_cancelled_notification', array( $this, 'trigger' ) );
	...
	function get_form_description() {
		$description = __( 'Cancelled order SMS are sent to the recipient list when orders have been marked cancelled (if they were previously processing or on-hold).', 'woocommerce' );
...
return new WC_SMS_Cancelled_Order();
